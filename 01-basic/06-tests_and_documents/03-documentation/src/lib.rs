...
impl SavingsAccount {
    /// Creates a `SavingsAccount` instance with a balance of 0
    /// 
    /// # Examples
    ///
    /// ```
    ...
    /// # Panics
    ///
    /// The function will panic if the initial balance is negative
    ///
    /// ```should_panic
    /// use bank::SavingsAccount;
    ///
    /// let mut account = SavingsAccount::new();
    /// account.deposit(-1); // panics
    /// ```
    ///
    /// # Returns
    ///
    /// A new `SavingsAccount` instance
    ///
